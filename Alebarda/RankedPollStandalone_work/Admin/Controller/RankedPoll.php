...
class RankedPoll extends AbstractController
{
    ...
    protected function pollAddEdit(\Alebarda\RankedPollStandalone\Entity\Poll $poll)
    {
        ...
        $viewParams = [
            ...
            'openDateTime' => $poll->open_date ? date('Y-m-d\TH:i', $poll->open_date) : '',
            'closeDateTime' => $poll->close_date ? date('Y-m-d\TH:i', $poll->close_date) : '',
        ];
    ...
    protected function pollSaveProcess(\Alebarda\RankedPollStandalone\Entity\Poll $poll)
    {
        ...
        if ($input['open_date']) {
            $openInput = str_replace('T', ' ', $input['open_date']);
            $openDate = strtotime($openInput);
        ...
        if ($input['close_date']) {
            $closeInput = str_replace('T', ' ', $input['close_date']);
            $closeDate = strtotime($closeInput);
